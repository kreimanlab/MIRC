...
# submirc vs submirc post: threshold used to sort temporal electrodes into "early" and "late" electrodes
threshold_temporal = 315
...
if True:
    ...
    # settings for full vs submirc
    conds_names = ('full', 'submirc')
...
print('{0} vs {1}: for {2} out of {3} electrodes a location could be determined'.
      format(conds_names[0], conds_names[1], len(electrode_time_begin_s), n_significant_electrodes))
...
print('Significance start: {0} vs {1}: Comparing Parietal (n={2}) and Occipital (n={3}) responses'.
      format(conds_names[0], conds_names[1], len(time_begin_ms_parietal), len(time_begin_ms_occipital)))
print(stats.mannwhitneyu(time_begin_ms_parietal, time_begin_ms_occipital))
print('')

# Parietal vs Temporal
print('Significance start: {0} vs {1}: Comparing Parietal (n={2}) and Temporal (n={3}) responses'.
      format(conds_names[0], conds_names[1], len(time_begin_ms_parietal), len(time_begin_ms_temporal)))
print(stats.mannwhitneyu(time_begin_ms_parietal, time_begin_ms_temporal))
print('')


# -------------------- DIPTEST (SIGNIFICANCE START) for unimodality of the distribution --------------------
# OCCIPITAL ELECTRODES
print('Significance start: {0} vs {1}: diptest - occipital electrodes (n={2})'.format(conds_names[0], conds_names[1], len(time_begin_ms_occipital)))
print('(val, pvalue)')
print(diptest.diptest(np.array(time_begin_ms_occipital)))
print('')

# TEMPORAL ELECTRODES
print('Significance start: {0} vs {1}: diptest - temporal electrodes (n={2})'.format(conds_names[0], conds_names[1], len(time_begin_ms_temporal)))
print('(val, pvalue)')
print(diptest.diptest(np.array(time_begin_ms_temporal)))
print('')

# PARIETAL ELECTRODES
print('Significance start: {0} vs {1}: diptest - parietal electrodes (n={2})'.format(conds_names[0], conds_names[1], len(time_begin_ms_parietal)))
print('(val, pvalue)')
print(diptest.diptest(np.array(time_begin_ms_parietal)))
print('')
...
if (conds_names[0] == 'mirc') and (diptest.diptest(np.array(time_begin_ms_frontal))[1] < 0.05):
...
# ----------------- Plot figures for SIGNIFICANCE START: ALL AREAS -----------------
y_label = 'onset of selectivity (ms)'

# Select data to plot
if conds_names[0] == 'mirc':
    # MIRC vs SUBMIRC
    tmp_x = np.concatenate((0 * np.ones(len(time_begin_ms_frontal)), 1 * np.ones(len(time_begin_ms_parietal)),
                            2 * np.ones(len(time_begin_ms_temporal)), 3 * np.ones(len(time_begin_ms_occipital))))
    tmp_y = np.concatenate((time_begin_ms_frontal, time_begin_ms_parietal, time_begin_ms_temporal, time_begin_ms_occipital))
elif conds_names[0] == 'full':
    # FULL vs SUBMIRC
    tmp_x = np.concatenate((0 * np.ones(len(time_begin_ms_frontal)), 1 * np.ones(1),
                            2 * np.ones(len(time_begin_ms_temporal)), 3 * np.ones(len(time_begin_ms_occipital))))
    tmp_y = np.concatenate(
        (time_begin_ms_frontal, [float('NAN')], time_begin_ms_temporal, time_begin_ms_occipital))
else:
    # SUBMIRC vs SUBMIRC post
    tmp_x = np.concatenate((0*np.ones(1), 1 * np.ones(len(time_begin_ms_parietal)), 2 * np.ones(len(time_begin_ms_temporal)),
                            3 * np.ones(len(time_begin_ms_occipital))))
    tmp_y = np.concatenate(([float('NAN')], time_begin_ms_parietal, time_begin_ms_temporal, time_begin_ms_occipital))

# ... and plot them
fig, axes = plt.subplots()
vplot = sns.violinplot(x=tmp_x, y=tmp_y, ax=axes, color='grey')
axes.set_xticks([0, 1, 2, 3], labels=['Frontal', 'Parietal', 'Temporal', 'Occipital'])
...
# now save figure
if save_figs == True:
    f_name = os.path.join(figures_dir, '{0}_vs_{1}_ALL_AREAS_violinplot_significance_start'.format(conds_names[0], conds_names[1]))
...
# ----------------- Plot figures for SIGNIFICANCE START: ONLY FRONTAL and TEMPORAL -----------------
# and only TEMPORAL for submirc_post vs submirc
# y_label = 'onset of selectivity (ms)'
y_label = 'start of selectivity (ms)'
# Select data to plot
if conds_names[0] == 'mirc':
    # we are plotting mirc vs submirc
    # tmp_x = np.concatenate((1*np.ones(len(time_begin_ms_temporal)), 2*np.ones(len(time_begin_ms_frontal))))
    # tmp_y = np.concatenate((time_begin_ms_temporal, time_begin_ms_frontal))
...
# now save figure
if save_figs == True:
    f_name = os.path.join(figures_dir, '{0}_vs_{1}_Frontal_Temporal_violinplot_significance_start'.format(conds_names[0], conds_names[1]))
    fig.savefig(fname=f_name + '.png', dpi=800, bbox_inches='tight')
    fig.savefig(fname=f_name + '.eps', dpi=800, bbox_inches='tight')
    fig.savefig(fname=f_name + '.svg', dpi=800, bbox_inches='tight')

# ----------------- Plot figures for SIGNIFICANCE START: ONLY FRONTAL and TEMPORAL, NO OCCIPITAL -----------------
# and only TEMPORAL for submirc_post vs submirc
# y_label = 'onset of selectivity (ms)'
y_label = 'start of selectivity (ms)'
# Select data to plot
...
# now save figure
if save_figs == True:
    f_name = os.path.join(figures_dir, '{0}_vs_{1}_Frontal_Temporal_NO_occipital_violinplot_significance_start'.format(conds_names[0], conds_names[1]))
    fig.savefig(fname=f_name + '.png', dpi=300, bbox_inches='tight')
    # fig.savefig(fname=f_name + '.eps', dpi=800, bbox_inches='tight')
    fig.savefig(fname=f_name + '.svg', dpi=300, bbox_inches='tight')


# ----------------- Plot figure for SIGNIFICANCE START: FRONTAL, TEMPORAL, OCCIPITAL -----------------
# and only TEMPORAL for submirc_post vs submirc
# y_label = 'onset of selectivity (ms)'
y_label = 'start of selectivity (ms)'
# Select data to plot
if conds_names[0] == 'mirc':
    # MIRC vs SUBMIRC
    tmp_x = np.concatenate((0 * np.ones(len(time_begin_ms_frontal)), 1 * np.ones(len(time_begin_ms_temporal)),
                            2 * np.ones(len(time_begin_ms_occipital))))
    tmp_y = np.concatenate((time_begin_ms_frontal, time_begin_ms_temporal, time_begin_ms_occipital))
elif conds_names[0] == 'full':
    # FULL vs SUBMIRC
    tmp_x = np.concatenate((0 * np.ones(len(time_begin_ms_frontal)), 1 * np.ones(len(time_begin_ms_temporal)),
                            2 * np.ones(len(time_begin_ms_occipital))))
    ...
    # SUBMIRC vs SUBMIRC post
    tmp_x = np.concatenate((0 * np.ones(1), 1 * np.ones(len(time_begin_ms_temporal)), 2 * np.ones(1)))
    tmp_y = np.concatenate(([float('NAN')], time_begin_ms_temporal, [float('NAN')]))

# ... and plot them
fig, axes = plt.subplots(figsize=(5, 5))
vplot = sns.violinplot(x=tmp_x, y=tmp_y, ax=axes, color='grey')
axes.set_xticks([0, 1, 2], labels=['Frontal', 'Temporal', 'Occipital'])
axes.set_ylabel(y_label, fontsize=24)
axes.tick_params(axis='y', labelsize=22)
...
# now save figure
if save_figs == True:
    f_name = os.path.join(figures_dir, '{0}_vs_{1}_Frontal_Temporal_Occipital_violinplot_significance_start'.format(conds_names[0], conds_names[1]))
    fig.savefig(fname=f_name + '.png', dpi=300, bbox_inches='tight')
    # fig.savefig(fname=f_name + '.eps', dpi=800, bbox_inches='tight')
    fig.savefig(fname=f_name + '.svg', dpi=300, bbox_inches='tight')
...
if conds_names[0] == 'mirc':
    ...
    #counts_F_early, bins_F_early = np.histogram(time_begin_ms_frontal_early, range=(50, 550), bins=20)
    #counts_F_late, bins_F_late = np.histogram(time_begin_ms_frontal_early, range=(50, 550), bins=20)
    ...
    # ------------------- plot distributions of frontal, parietal and occipital significance start -------------------
    # first order times by latency
    tmp_1 = time_begin_ms_frontal[time_begin_ms_frontal < threshold_frontal]
    counts_F, bins_F = np.histogram(tmp_1, range=(50, 550), bins=15)
    counts_T, bins_T = np.histogram(time_begin_ms_temporal, range=(50, 550), bins=15)
    counts_O, bins_O = np.histogram(time_begin_ms_occipital, range=(50, 550), bins=12)

    fig, axes = plt.subplots(3, 1)
    axes[0].hist(tmp_1, bins=bins_F, density=False, color='black')
    x_dist = np.median(tmp_1)
    axes[0].plot([x_dist, x_dist], [0, 0.01], linewidth=3, linestyle='--', color='green')
    axes[0].set_xlim([50, 550])
    # axes[0].set_ylim([0, 0.01])
    axes[0].set_ylabel('Frontal', fontsize=18)
    axes[0].tick_params(axis='both', labelsize=18)

    axes[1].hist(time_begin_ms_temporal, bins=bins_T, density=False, color='black')
    x_dist = np.median(time_begin_ms_temporal)
    axes[1].plot([x_dist, x_dist], [0, 0.01], linewidth=3, linestyle='--', color='green')
    axes[1].set_xlim([50, 550])
    # axes[1].set_ylim([0, 0.01])
    axes[1].set_ylabel('Temporal', fontsize=18)
    axes[1].tick_params(axis='both', labelsize=18)

    axes[2].hist(time_begin_ms_occipital, bins=bins_O, density=False, color='black')
    x_dist = np.median(time_begin_ms_occipital)
    axes[2].plot([x_dist, x_dist], [0, 0.01], linewidth=3, linestyle='--', color='green')
    axes[2].set_xlim([50, 550])
    # axes[2].set_ylim([0, 0.01])
    axes[2].set_ylabel('Occipital', fontsize=18)
    axes[2].tick_params(axis='both', labelsize=18)
    axes[2].set_xlabel('onset of significance (ms)', fontsize=20)
    plt.tight_layout()

    if save_figs == True:
        # now save figure
        f_name = os.path.join(figures_dir, '{0}_vs_{1}_Frontal_Temporal_Occipital_histogram_significance_start'.format(conds_names[0],
                                                                                                           conds_names[1]))
        fig.savefig(fname=f_name + '.png', dpi=300, bbox_inches='tight')
        # fig.savefig(fname=f_name + '.eps', dpi=800, bbox_inches='tight')
        fig.savefig(fname=f_name + '.svg', dpi=800, bbox_inches='tight')

    # -------------------- plot violinplots of frontal "early" and temporal distributions --------------------
    time_begin_ms_frontal_early = time_begin_ms_frontal[time_begin_ms_frontal < threshold_frontal]
    time_begin_ms_frontal_late = time_begin_ms_frontal[time_begin_ms_frontal >= threshold_frontal]

    # we are plotting mirc vs submirc
    tmp_y = np.concatenate((time_begin_ms_frontal_early, time_begin_ms_frontal_late, time_begin_ms_temporal))
    tmp_x = len(time_begin_ms_frontal_early) * ['early'] + len(time_begin_ms_frontal_late) * ['late'] +\
            len(time_begin_ms_temporal) * ['temporal']
    my_palette = {'early': color_frontal['early'], 'late': color_frontal['late'], 'temporal': 'grey'}

    # ... and plot them
    fig, axes = plt.subplots(figsize=(5,5))
    vplot = sns.violinplot(x=tmp_x, y=tmp_y, ax=axes, palette=my_palette)
    axes.set_xticks([0, 1, 2], labels=['Frontal\nearly', 'Frontal\nlate', 'Temporal'])
    axes.set_ylabel('onset of selectivity (ms)', fontsize=24)
    axes.tick_params(axis='y', labelsize=22)
    axes.tick_params(axis='x', labelsize=16)
    # axes.set_xlim(-0.5, 1.5)
    axes.set_ylim(0, 600)
    ...
    if save_figs == True:
        # now save figure
        f_name = os.path.join(figures_dir, '{0}_vs_{1}_Early_Late_Frontal_Temporal_VIOLINPLOT_significance_start'.format(conds_names[0],
                                                                                                           conds_names[1]))
        fig.savefig(fname=f_name + '.png', dpi=800, bbox_inches='tight')
        fig.savefig(fname=f_name + '.eps', dpi=800, bbox_inches='tight')
        fig.savefig(fname=f_name + '.svg', dpi=800, bbox_inches='tight')
...
if conds_names[0] == 'submirc':
    ...
    # extract bins for frontal electrodes
    count_temporal, bins_temporal = np.histogram(time_begin_ms_temporal, range=(50, 550), bins=10)
    time_begin_ms_temporal_early = time_begin_ms_temporal[time_begin_ms_temporal < threshold_temporal]
    time_begin_ms_temporal_late = time_begin_ms_temporal[time_begin_ms_temporal >= threshold_temporal]
    #counts_F_early, bins_F_early = np.histogram(time_begin_ms_frontal_early, range=(50, 550), bins=20)
    #counts_F_late, bins_F_late = np.histogram(time_begin_ms_frontal_early, range=(50, 550), bins=20)

    # set colors to plot
    color_temporal = {'early': [1, 0.65, 0], 'late': [0.2, 0.8, 0.2]}

    # ----------------- plot HISTOGRAM of the responses of frontal electrodes ONE COLOR -----------------
    fig, axes = plt.subplots()
    axes.hist(time_begin_ms_temporal, bins=bins_temporal, color='black')
    axes.set_ylabel('# of electrodes', fontsize=22)
    axes.set_yticks(range(0, 7, 2))
    axes.set_ylim([0, 6])
    axes.set_xlabel('onset of selectivity (ms)', fontsize=22)
    axes.tick_params(axis='both', labelsize=20)
    plt.tight_layout()
    ...
    if save_figs == True:
        # now save figure
        f_name = os.path.join(figures_dir,
                              '{0}_vs_{1}_Temporal_HISTOGRAM_ONE_COLOR_significance_start'.format(conds_names[0],
                                                                                                 conds_names[1]))
        fig.savefig(fname=f_name + '.png', dpi=800, bbox_inches='tight')
        fig.savefig(fname=f_name + '.eps', dpi=800, bbox_inches='tight')
        fig.savefig(fname=f_name + '.svg', dpi=800, bbox_inches='tight')
    ...
    # ----------------- plot HISTOGRAM of the responses of frontal electrodes TWO COLORS -----------------
    fig, axes = plt.subplots()
    axes.hist(time_begin_ms_temporal_early, bins=bins_temporal, color=color_temporal['early'], label='median={0:3.0f}ms'.format(np.median(time_begin_ms_temporal_early)))
    axes.hist(time_begin_ms_temporal_late, bins=bins_temporal, color=color_temporal['late'], label='median={0:3.0f}ms'.format(np.median(time_begin_ms_temporal_late)))
    axes.plot([np.median(time_begin_ms_temporal_early), np.median(time_begin_ms_temporal_early)], [0, 6], linewidth=3, linestyle='--', color='black')
    axes.plot([np.median(time_begin_ms_temporal_late), np.median(time_begin_ms_temporal_late)], [0, 6], linewidth=3, linestyle='--', color='black')
    axes.set_ylabel('# of electrodes', fontsize=22)
    axes.legend(fontsize=15, loc='center left')
    ...
    if save_figs == True:
        # now save figure
        f_name = os.path.join(figures_dir, '{0}_vs_{1}_TEMPORAL_HISTOGRAM_significance_start_TWO_COLORS'.format(conds_names[0], conds_names[1]))
        fig.savefig(fname=f_name + '.png', dpi=800, bbox_inches='tight')
        fig.savefig(fname=f_name + '.eps', dpi=800, bbox_inches='tight')
        fig.savefig(fname=f_name + '.svg', dpi=800, bbox_inches='tight')

    # ------------------- plot sorted onset of significance start vs subject number -------------------
    sort_inds = np.argsort(time_begin_ms_temporal)

    fig, axes = plt.subplots(1, 2)
    # plot sorted onset of significance
    axes[0].plot(np.array(time_begin_ms_temporal)[sort_inds], marker='o', linestyle='None')
    axes[0].set_ylabel('onset of significance (ms)', fontsize=20)
    axes[0].set_xlabel('electrode #', fontsize=20)
    axes[0].tick_params(axis='both', labelsize=18)
    # plot subject number
    axes[1].plot(np.array(temporal_electrodes_subj_num)[sort_inds], marker='o', linestyle='None')
    axes[1].set_ylabel('subject number', fontsize=20)
    axes[1].set_xlabel('electrode #', fontsize=20)
    axes[1].tick_params(axis='both', labelsize=18)
    plt.tight_layout()
    ...
    if save_figs == True:
        # now save figure
        f_name = os.path.join(figures_dir, '{0}_vs_{1}_Temporal_sorted_significance_start_VS_subjs'.format(conds_names[0], conds_names[1]))
        fig.savefig(fname=f_name + '.png', dpi=800, bbox_inches='tight')
        fig.savefig(fname=f_name + '.eps', dpi=800, bbox_inches='tight')
        fig.savefig(fname=f_name + '.svg', dpi=800, bbox_inches='tight')


    # ----------------- Plot figures for SIGNIFICANCE START: ONLY TEMPORAL, NO OCCIPITAL. FRONTAL -----------------
    # and only TEMPORAL for submirc_post vs submirc
    # y_label = 'onset of selectivity (ms)'
    y_label = 'start of selectivity (ms)'
    # Select data to plot
...
# ---------------------------------------------------------------------------
# if we are processing the full vs submirc contrast then we save additional plots
#
if conds_names[0] == 'full':
    # extract bins for frontal electrodes
    count_temporal, bins_temporal = np.histogram(time_begin_ms_temporal, range=(50, 550), bins=10)
    time_begin_ms_temporal_early = time_begin_ms_temporal[time_begin_ms_temporal < threshold_temporal]
    time_begin_ms_temporal_late = time_begin_ms_temporal[time_begin_ms_temporal >= threshold_temporal]
    ...
    axes.hist(time_begin_ms_temporal_early, bins=bins_temporal, color=color_temporal['early'], label='median={0:3.0f}ms'.format(np.median(time_begin_ms_temporal_early)))
    axes.hist(time_begin_ms_temporal_late, bins=bins_temporal, color=color_temporal['late'], label='median={0:3.0f}ms'.format(np.median(time_begin_ms_temporal_late)))
    axes.plot([np.median(time_begin_ms_temporal_early), np.median(time_begin_ms_temporal_early)], [0, 15], linewidth=3, linestyle='--', color='black')
    axes.plot([np.median(time_begin_ms_temporal_late), np.median(time_begin_ms_temporal_late)], [0, 15], linewidth=3, linestyle='--', color='black')
    axes.set_ylabel('# of electrodes', fontsize=22)
    axes.legend(fontsize=15, loc='center left')
    axes.set_yticks(range(0, 10, 2))
    axes.set_ylim([0, 15])
