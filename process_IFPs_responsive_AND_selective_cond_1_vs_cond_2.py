...
# conds_compare_interval_s = [0.05, 0.3]
...
def compute_n_significant(args_in):
    ...
    # Initialize output variables
    # n_cond_1_vs_cond_2 = 0
    # responsive_electrodes_name = []
    # responsive_electrodes_pos = np.zeros((IFPs.shape[0], 3))
    # -- responsive_electrodes_hemisphere = np.zeros(IFPs.shape[0])
    # responsive_electrodes_area = []
    # responsive_electrodes_macro_area = []
    # time_begin_ms = []
    # time_end_ms = []
    ...
    # This is true ONLY if there is only ONE contrast. In case we shuffle the trial
    # if np.sum(s_cond_1_trial_inds_orig)+np.sum(s_cond_2_trial_inds_orig) != np.sum(cond_1_trial_inds_orig) + np.sum(cond_2_trial_inds_orig):
    #    raise Warning('SHUFFLING: sum of indices of shuffled trials is not equal to sum of indices of NON shuffled trials')
    ...
    for curr_electrode_ind in range(0, subj_electrodes_num):
        ...
        if curr_electrode_ind in sel_electrodes_inds:
            # We do not want to print a lot of stuff on the console if we are performing a shuffling analysis
            #if shuffle_trials == False:
            # print('subject {0} - electrode {1} - {2} trials={3} - {4} trials={5}'. \
            #      format(curr_subj_str, channel_names[curr_electrode_ind], conds_names[0], len(cond_1_trial_inds), \
            #             conds_names[1], len(cond_2_trial_inds)))


            # use NON-PARAMETRIC statistics
            # This is WAY slower (a factor of 100) than t-test but it uncovers many more significant electrodes
    ...
    # return responsive_selective_n_chans, responsive_selective_chans_name, responsive_selective_time_begin_ms, \
    #    responsive_selective_time_end_ms, cond_1_trial_inds, cond_2_trial_inds, \
    #       cond_1_trial_inds_orig, cond_2_trial_inds_orig, responsive_selective_chans_pos, responsive_electrodes_pos, \
    #        responsive_selective_chans_area, responsive_selective_chans_macro_area, res_responsive_selective
...
if __name__ == '__main__':
    ...
    for curr_shuffle_ind in range(0, n_shufflings):
        ...
        # for curr_subj_ind in range(0, 2):
            ...
            # Here I perform a shuffle
            '''if shuffle_trials == True:
                # random.shuffle(behavior_data.RelabeledCond_Tonino)
                curr_subj_behavioral_codes = behavior_data.RelabeledCond_Tonino.copy()
                inds = [curr_ind for curr_ind in range(0, len(behavior_data.RelabeledCond_Tonino)) if
                        behavior_data.RelabeledCond_Tonino[curr_ind] in conds_codes_for_shuffling]
                inds_shuffled = inds.copy()
                random.shuffle(inds_shuffled)
                curr_subj_behavioral_codes[inds] = curr_subj_behavioral_codes[inds_shuffled]
            else:
            ...
            if useParallel == False:
                ...
                print('----------------- n electrodes = {}'.format(res_responsive_selective.n_chans))
        ...
        if useParallel == True:
            for curr_contrast_ind in range(0, n_contrasts):
                ...
                #n_cond_1_vs_cond_2, responsive_electrodes, time_begin_ms, time_end_ms, cond_1_trial_inds, cond_2_trial_inds, \
                #all_cond_1_trial_inds, all_cond_2_trial_inds = pool.map(compute_n_significant, all_pars)
                ...
                print('----------------- n electrodes = {}'.format(n_chans_subj))
    ...
    # final results with the number of significant and selective channels for each subject
    for curr_contrast_ind in range(0, n_contrasts):
        print('{0} vs {1} - #electrodes={2}'.format(conds_names[curr_contrast_ind][0], conds_names[curr_contrast_ind][1], n_cond_1_vs_cond_2_subjs[curr_contrast_ind]))
    ...
    if False:
        # --------------------------------
        print('{0}\t'.format(cond_1_vs_cond_2_electrodes_subjs[curr_contrast_ind][curr_subj_ind][curr_electrode_ind]))

        print('{0}\t{1:4.2f}\t{2:4.2f}\t{3}\t{4}\t{5}\t{6}'.format(\
            cond_1_vs_cond_2_electrodes_subjs[curr_contrast_ind][curr_subj_ind][curr_electrode_ind], \
            time_begin_ms_subjs[curr_contrast_ind][curr_subj_ind][curr_electrode_ind],\
            time_end_ms_subjs[curr_contrast_ind][curr_subj_ind][curr_electrode_ind], \
            responsive_electrodes_macro_area_subjs[curr_contrast_ind][curr_subj_ind][curr_electrode_ind], \
            '{0}\t{1}\t{2}'.format(responsive_electrodes_pos_subjs[curr_contrast_ind][curr_subj_ind][curr_electrode_ind][0], \
                     responsive_electrodes_pos_subjs[curr_contrast_ind][curr_subj_ind][curr_electrode_ind][1], \
                     responsive_electrodes_pos_subjs[curr_contrast_ind][curr_subj_ind][curr_electrode_ind][2]), \
            responsive_electrodes_area_subjs[curr_contrast_ind][curr_subj_ind][curr_electrode_ind], \
            int(responsive_electrodes_hemisphere_subjs[curr_contrast_ind][curr_subj_ind][curr_electrode_ind])\
            ))

        print('{0:4.2f}'.format(time_begin_ms_subjs[curr_contrast_ind][curr_subj_ind][curr_electrode_ind]))